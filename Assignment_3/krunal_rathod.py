# -*- coding: utf-8 -*-
"""ML_2023_assignment_3.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1uy2VmWWH7StHxa5TpJD2OQlMvV48cTwq
...
class Linear(Layer):
    ...
    def backward(self, grad_in: np.ndarray) -> Layer.BackwardResult:
        ...
        db = np.ones(grad_in.shape[0]) @ grad_in

        d_inputs = np.dot(grad_in, self.var["W"].T)
...
class SoftmaxCrossEntropy(Loss):
    def forward(self, inputs: np.ndarray, targets: np.ndarray) -> float:
        ...
        exp_input = np.exp(y)
        softmax =  exp_input / exp_input.sum(axis=1).reshape(n, 1)
...
def train_one_step(model: Layer, loss: Loss, learning_rate: float, input: np.ndarray, target: np.ndarray, weight_decay=0.0005) -> float:
    ...
    # Calculate the prediction and loss value
    prediction = model.forward(input)
    loss_value = loss.forward(prediction, target)

    # Calculate the loss value with weight decay
    weight_decay_term = 0
    for param in model.var.values():
        if param is not None:
            weight_decay_term = 0.5 * sum(np.sum(np.square(param[0]['W'])) + np.sum(np.square(param[0]['b'])) for param in model.var.values())


    loss_value += weight_decay * weight_decay_term

    # Perform backpropagation
    grad_pred = loss.backward()
    variable_grads = model.backward(grad_pred).variable_grads

    # Update weights and biases using gradient descent with weight decay
    for key, grad in variable_grads.items():
        if grad is None:
            continue
        weight_decay_grad = weight_decay * model.var[key]
        model.var[key] -= learning_rate * (grad + weight_decay_grad)
...
def verify(images: np.ndarray, targets: np.ndarray) -> Tuple[int, int]:
    ...
    n = images.shape[0]
    predictions = predictions / predictions.sum(axis=1).reshape(n,1)

    max_n = 1

    max_pred = predictions * (predictions >= np.sort(predictions, axis=1)[:,[-max_n]]).astype(int)

    mask = max_pred!=0
    final_pred= np.where(mask.any(axis=1), mask.argmax(axis=1), 0)

    num_ok= np.sum(final_pred==targets)
...
def validate() -> float:
    ...
    ## Implement. Use the verify() function to verify your data.
    validation_image_set = train_validation_set.images[validation_indices]
    validation_label_set = train_validation_set.labels[validation_indices]

    for i in range(0, validation_indices.shape[0], batch_size):
        images = validation_image_set[i:i + batch_size]
        labels = validation_label_set[i:i + batch_size]

        accumulated_ok+=verify(images, labels)[0]
        count+=batch_size
...
for epoch in range(200):
    ...
    for i in range(0, len(train_validation_set), batch_size):
        ...
        train_images_batch = train_validation_set.images[batch_indices]
        train_labels_batch = train_validation_set.labels[batch_indices]

        # Train for multiple steps per epoch
        for step in range(5):
            current_loss_value += train_one_step(model, loss, learning_rate, train_images_batch, train_labels_batch)

    # Validate at the end of each epoch
    validation_accuracy = validate()

    print(f"Epoch {epoch}: loss: {current_loss_value}, validation accuracy: {validation_accuracy:.2f}")
    ...
    if validation_accuracy > best_validation_accuracy:
        ...
        best_epoch = (epoch)
        print(f"\tThe epoch {best_epoch} has the current best accuracy equal to {best_validation_accuracy:.2f}. We wait {(epoch) - best_epoch} iterations for having it.")
    else:
        if (epoch) - best_epoch > 10:
            print(f"\tThe current best accuracy didn't change after {best_epoch} epoch.")
            break

    # End

print(f"Test set performance: {test():.2f}")
